...
class BaseTestNodes(object):
    ...
    def test_find_methods(self):
        ...
        elems = self.xml4h_root.find('Element1')
        ...
        self.assertEqual(self.elem1,
                self.xml4h_root.find_first('Element1').impl_node)
    ...
    def test_xpath(self):
        ...
        self.assertEqual([], elems)
        # Find non-namespaced element
        elems = self.xml4h_root.xpath('//Element1')
        self.assertEqual(self.xml4h_root.find('Element1'), elems)
        # Find multiple non-namespaced elements
        ...
        self.assertEqual([self.xml4h_root.Element4], elems)
        # Lookup attribute
        result = self.xml4h_root.xpath('//Element1/@a',
            namespaces={'x': 'urn:ns2'})
        self.assertEqual(['1'], result)
        # Lookup text
        result = self.xml4h_root.xpath('//Element1/text()',
            namespaces={'x': 'urn:ns2'})
        self.assertEqual(['Some text'], result)
        # sum() aggregate function
        result = self.xml4h_root.xpath('sum(//Element1/@*)',
            namespaces={'x': 'urn:ns2'})
